...
def run_probe():
    ...
    for name, bset in blur_settings.items():
        ...
        for tampered_img_path, mask_path, orig_img_path in gen_list:
            ...
            # Wait, dataloader resizes mask using F.adaptive_max_pool2d.
            m_img_t_tmp = torch.from_numpy(m_img).float().unsqueeze(0).unsqueeze(0)
            m_img = F.adaptive_max_pool2d(m_img_t_tmp, (256, 256)).squeeze().numpy().astype(np.uint8)
